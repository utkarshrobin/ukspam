...
from uk_spam.data import RAID, REPLYRAID, uk_spam, MRAID, SRAID, CRAID, uk_spam
...
async def _(e):
    ...
    global que    
    if e.sender_id in SUDO_USERS:
        uk_spam = ("".join(e.text.split(maxsplit=1)[1:])).split(" ", 1)

        if len(e.text) > 12:
            message = str(uk_spam[0])
            a = await e.client.get_entity(message)
            g = a.id
            try:
                queue = que.get(g)
...
async def spam(e):
    ...
    if e.sender_id in SUDO_USERS:
        ...
        if len(MKsr) == 2:
            ...
            await e.reply(usage, parse_mode=None, link_preview=None )
async def spam(e):
    usage = f"𝗠𝗼𝗱𝘂𝗹𝗲 𝗡𝗮𝗺𝗲: UKOWNER \n  » {hl}ukowner <count> <Username of User>\n  » {hl}ukowner <count> <reply to a User>"
    if e.sender_id in SUDO_USERS:
        MKsr = ("".join(e.text.split(maxsplit=1)[1:])).split(" ", 1)
        await e.get_reply_message()
        if len(MKsr) == 2:
